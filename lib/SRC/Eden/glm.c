...
//
//  Updated by Kaicheng Yu (KCY) yukaicheng@hotmail.com. All rights reserved.
//  1.8.0   2016-01-07  KCY     Support load stl file to glmModel.
...
// Created by KCY
/** Utilities for STL reader */
static GLuint
readInt32(FILE* file) {

    unsigned char buf[4];
    // This would be interpreted by return function, when read the binary data.
    // char buf[5];    // append null by fgets. Could also be substituted by fread(buf,1,4,file)
    // fgets((char *)buf, 4, file);
    fread(buf, sizeof(unsigned char), 4, file);
    return ( buf[0]) | (( buf[1]) << 8) | (( buf[2]) << 16) | (( buf[3]) << 24 );
}

static GLfloat*
readVector3(FILE* file){
    union{
        GLuint i;
        GLfloat f;
    } val;
    GLfloat* vector = (GLfloat*) malloc(sizeof(GLfloat) * 3);
    val.i = readInt32(file);
    vector[0] = val.f;
    val.i = readInt32(file);
    vector[1] = val.f;
    val.i = readInt32(file);
...
/*
 * glmSTLFirstPass: first pass is the function to read STL file, gets all related
 * statistics of model, such as number of normals, vertices, triangles ...
 * Also allocateds the memory in each groups for triangle _indices_
 *
 * model - properly initialized GLMmodel structure
 * file  - (fopen'd) file descriptor
 * contextindex - context index refered to the current OpenGL context !
 *
 */
static GLvoid
glmSTLFirstPass(GLMmodel* model, FILE* file, const int contextIndex) {
    ...
    /* read comment 80 byte */
    char comment[81];
    fgets(comment, 80, file);
    comment[80] = 0;
    model->comments = comment;
    //  Get the number of triangles.
    //  char comments2[81];
    //  fgets(comments2, 80, file);
    model->numtriangles = numtriangles = readInt32(file);
    
    if(fileSize != (84 + numtriangles * 50)) {
        EDEN_LOGe("glmSTLFirstPass: Error: File format is wrong. %li expected %u \n",fileSize,(84 + numtriangles * 50));
        EDEN_LOGe("glmSTLFirstPass: Error: Print comment and the number of triangles. %s \n %u \n",comment,numtriangles);
    //    EDEN_LOGe("glmSTLFirstPass: Comparision %s \n", comments2);
...
/*
 * glmSTLSecondPass: read all related data with model allocated space.
 *
 * file - file descriptor
 * model - model with allocated space.
 */

static GLvoid
glmSTLSecondPass(GLMmodel* model, FILE* file){
    GLuint      numvertices = 1;
    GLuint      numnormals  = 1;
    ...
    if(model->numtriangles > 0){
        for(GLuint i=0; i < model->numtriangles;i++){
            GLfloat* normal = readVector3(file);
            normals[numnormals * 3 + 0] = normal[0];
            normals[numnormals * 3 + 1] = normal[1];
            normals[numnormals * 3 + 2] = normal[2];
            numnormals++;
            for(GLuint j = 0; j < 3; j++){
                vertices[numvertices*3*j + 0] = readInt32(file);
                vertices[numvertices*3*j + 1] = readInt32(file);
                vertices[numvertices*3*j + 2] = readInt32(file);
            }
            numvertices += 3;
        }
    }
...
/*
 * glmReadSTL: Reads a model description from a .STL file.
 * Returns a pointer to the created object which should be free'd with
 * glmDelete()
 
 * filename - name of the file containing the .STL format data.
 * cont
 */

GLMmodel*
glmReadSTL(const char *filename, const int contextIndex) {
    GLMmodel*   model;
    FILE*       file;
    
    file = fopen(filename, "r");
    if(!file){
        EDEN_LOGe("glmReadSTL failed: cannot open the file %s", filename);
        return(NULL);
    }
    ...
    model->flipTextureV        = FALSE;
    
    glmSTLFirstPass(model, file, contextIndex);
    EDEN_LOG("glmReadSTL: vertices %d, triangles %d, normals %d. \n" , model->numvertices, model->numtriangles, model->numnormals);
